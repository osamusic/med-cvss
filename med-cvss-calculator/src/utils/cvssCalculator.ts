...
function calculateEnvironmentalScore(_baseScore: number, vector: CVSSVector): number {
  ...
  if (vector.S === 'U') {
    ...
    modifiedImpactScore = 7.52 * (modifiedImpact - 0.029) - 3.25 * Math.pow(modifiedImpact - 0.02, 15);
  }
  ...
  if (vector.S === 'U') {
    environmentalScore = roundUp(
      Math.min((modifiedImpactScore + modifiedExploitability) * 
        getMetricScore('E', vector.E || 'X') * 
        getMetricScore('RL', vector.RL || 'X') * 
        getMetricScore('RC', vector.RC || 'X'), 10)
    );
  } else {
    environmentalScore = roundUp(
      Math.min(1.08 * (modifiedImpactScore + modifiedExploitability) * 
        getMetricScore('E', vector.E || 'X') * 
        getMetricScore('RL', vector.RL || 'X') * 
        getMetricScore('RC', vector.RC || 'X'), 10)
    );
