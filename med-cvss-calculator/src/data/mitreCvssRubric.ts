...
export const mitreRubricQuestions: MitreQuestion[] = [
  // ATTACK VECTOR - Extended Medical Device Assessment
  {
    ...
    guidance:
      'Consider any network protocols including IP, TCP/UDP, wireless, or communication interfaces',
    options: [
      {
        value: 'yes',
  ...
  // USER INTERACTION - Extended Medical Device Assessment
  {
    ...
    guidance:
      'Consider if exploitation requires user actions like clicking, opening files, or system interactions',
    options: [
      {
        value: 'no',
  ...
  // SCOPE - Extended Medical Device Assessment
  {
    ...
    guidance:
      'Consider if exploitation can affect other components with different authorization boundaries',
    options: [
      {
        value: 'no',
  ...
  // CONFIDENTIALITY IMPACT - PHI/PII Data
  {
    ...
    guidance:
      'Consider if attacker can access patient personal information, medical records, or identifying data',
    options: [
      {
        value: 'yes',
  ...
  // INTEGRITY IMPACT - PHI/PII Data
  {
    ...
    type: 'decision',
    description:
      'Patient Health Information / Personally Identifiable Information modification/deletion',
    guidance:
      'Consider if attacker can alter or delete patient personal information or medical records',
    options: [
      {
        value: 'yes',
  ...
  // AVAILABILITY IMPACT - PHI/PII Data
  {
    ...
    guidance:
      'Consider if attacker can make patient personal information or medical records unavailable',
    options: [
      {
        value: 'yes',
...
export const calculateMitreVector = (answers: MitreRubricAnswers): { [key: string]: string } => {
  ...
  if (answers.XCP) {
    if (answers.XCP === 'yes') {
      ...
      if (answers.XCPM) {
        confidentialityImpacts.XCP =
          answers.XCPM === 'yes' || answers.XCPM === 'unknown' ? 'H' : 'L';
      } else {
  ...
  const availabilityImpactValues = Object.values(availabilityImpacts).filter(
    (v) => v !== undefined
  );
